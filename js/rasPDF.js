	        ...
	        for(var h=0; h < nodesArray.length; h++) {
	            ...
	            if (node.color != null) {
				    ...
				    */
			    	...
			    	console.log("Text cannot be drawn for "+ node.key);
			    }
...
/*				
	    ...
	    function canvasClick(e) {
	      ...
	      for(var i = nodesArray.length-1; i>=0; i--) {
	          ...
	          var distanceFromCenter = Math.sqrt(Math.pow(node.x*width - clickX, 2)
	            + Math.pow(node.y*height - clickY, 2))
	          // 判断被单击的点在哪个结点中
	      ...
	      if (isDragging == true) {
	          ...
	          if (previousSelectedNode != null) {
		          ...
		          if ((Math.pow((x*width-width/2),2)/3000+85) < y*height & y*height < (Math.pow((x*width-width/2),2)/6000+600)) {
		          		// 判断结点是否在组内
			          if (previousSelectedNode.group != null) {
				 		  // 计算被拖拽结点与鼠标位置差
			              ...
			              // 查找与被单击的结点同组的结点
			              for (var g=0; g<previousSelectedNode.group.length; g++) {
